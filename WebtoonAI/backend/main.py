...
async def transform_image(file: UploadFile = File(...), model_name: str = Form(...)):
    try:
        ...
        output_path = ""
        try:
            converted_filename = process_image(upload_path, model_name, output_path)
            conversion_result = "Success"

        except ValueError as e:
            print(f"main.py::Error in process_image: {e}")
        ...
        # 변환된 파일의 원래 위치와 대상 경로를 정확히 지정 
            ...
            source_path = converted_filename
            converted_path = converted_filename

            # 경로를 출력하여 디버깅에 도움
            print(f"converted_filename:::Source path: {source_path}, Target path: {converted_path}")
        ...
        if converted_path:
            return {"original_filename": upload_filename,
